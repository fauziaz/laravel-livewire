{{-- resources/views/livewire/mutasi/index.blade.php --}}
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                Daftar Mutasi Barang
            </h2>
            <a href="{{ route('riwayat-mutasi.create') }}"
                ...
                <div class="p-6 bg-white">
                    @if($riwayatmutasi->count())
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach($riwayatmutasi as $item)
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4 text-sm text-gray-900">
                                            {{ $item->lokasiAsal->nama_lokasi ?? '-' }} — Gedung {{ $item->lokasiAsal->gedung ?? '-' }}
                                        </td>
                                        <td class="px-6 py-4 text-sm text-gray-900">
                                            {{ $item->lokasiTujuan->nama_lokasi ?? '-' }} — Gedung {{ $item->lokasiTujuan->gedung ?? '-' }}
                                        </td>
                                        <td class="px-6 py-4 text-sm text-gray-900">
                                            {{ $item->status ?? '-' }}
                                        </td>
                                        ...
                                        <td class="px-6 py-4 text-sm text-gray-900 text-center">
                                            <div class="inline-flex space-x-2">
                                                {{-- Edit --}}
                                                <a href="{{ route('riwayat-mutasi.edit', $item->id) }}"
                                                ...
                                                <button
                                                    type="button"
                                                    wire:click="deleteMutasi({{ $item->id }})"
                                                    wire:confirm="Yakin ingin menghapus mutasi ini?"
                        ...
                        <div class="px-6 py-3 bg-white border-t border-gray-200 flex items-center justify-between">
                            <p class="text-sm text-gray-700">
                                Menampilkan {{ $riwayatmutasi->firstItem() }}-{{ $riwayatmutasi->lastItem() }} dari {{ $riwayatmutasi->total() }}
                            </p>
                            <div>
                                {{ $riwayatmutasi->links() }}
                            </div>
                    ...
                    @else
                        <div class="py-12 text-center">
                            <p class="text-gray-500">Belum ada data mutasi.</p>
                        </div>
                    @endif
                </div>
            </div>
